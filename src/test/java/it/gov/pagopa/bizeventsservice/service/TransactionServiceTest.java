...
import static it.gov.pagopa.bizeventsservice.util.ViewGenerator.*;
...
public class TransactionServiceTest {
    ...
    void setUp() {
    	transactionService = spy(new TransactionService(
                bizEventsViewGeneralRepository,
                bizEventsViewCartRepository,
                bizEventsViewUserRepository,
                bizEventsViewUserQueryRepository,
                receiptClient,
                generateReceiptClient,
                bizEventsService,
                cacheService
        ));
    ...
    void transactionViewUserDisabled() {
        ...
        when(bizEventsViewUserRepository.getBizEventsViewUserByTaxCodeAndTransactionIdAndHidden(USER_TAX_CODE_WITH_TX, TRANSACTION_ID, false))
                .thenReturn(viewUserList);
        Assertions.assertDoesNotThrow(() -> transactionService.updateBizEventVisibility(
                ViewGenerator.USER_TAX_CODE_WITH_TX, ViewGenerator.TRANSACTION_ID, true));
    ...
    void transactionViewUserDisabledEmptyList() {
        ...
        when(bizEventsViewUserRepository.getBizEventsViewUserByTaxCodeAndTransactionIdAndHidden(USER_TAX_CODE_WITH_TX, TRANSACTION_ID, false))
                .thenReturn(viewUserList);
        Assertions.assertThrows(AppException.class, () -> transactionService.updateBizEventVisibility(
                ViewGenerator.USER_TAX_CODE_WITH_TX, ViewGenerator.TRANSACTION_ID, true));
    ...
    void transactionViewUserCartDisabledPayer() {
        ...
        Assertions.assertDoesNotThrow(() -> transactionService.updateBizEventVisibility(
                ViewGenerator.USER_TAX_CODE_WITH_TX, TRANSACTION_ID_ON_CART_FOR_PAYER, true));
    ...
    void transactionViewUserCartDisabledDebtor() {
        ...
        when(bizEventsViewUserRepository.findByFiscalCodeAndTransactionIdAndEventIdAndHidden(USER_TAX_CODE_WITH_TX,TRANSACTION_ID, EVENT_ID, false))
                .thenReturn(viewUserList);

        Assertions.assertDoesNotThrow(() -> transactionService.updateBizEventVisibility(
                ViewGenerator.USER_TAX_CODE_WITH_TX, TRANSACTION_ID_ON_CART_FOR_DEBTOR, true));
    ...
    private void mockBizEventsViewUserQueryPage(List<BizEventsViewUser> listOfViewUser, String nextContinuationToken) {
        CosmosQueryPage<BizEventsViewUser> pageOfViewUser =
                new CosmosQueryPage<>(listOfViewUser, nextContinuationToken);

        when(bizEventsViewUserQueryRepository.findByTaxCodeAndOptionalFilters(
                eq(ViewGenerator.USER_TAX_CODE_WITH_TX),
                eq(false),
                isNull(),
                isNull(),
    ...
    private void verifyBizEventsViewUserQueryRepositoryCalled() {
        verify(bizEventsViewUserQueryRepository).findByTaxCodeAndOptionalFilters(
                eq(ViewGenerator.USER_TAX_CODE_WITH_TX),
                eq(false),
                isNull(),
                isNull(),
